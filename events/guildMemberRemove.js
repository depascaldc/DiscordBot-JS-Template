...
// CONFIG EXAMPLE
{
    ...
    if (config.channels.member_log_channel.enabled) {
        ...
        let message = config.channels.member_log_channel.message_left
        if (message) {
    ...
    if (config.channels.member_log_channel.enabled) {
        ...
        if (cachedChannel) {
            client.tools.sendEmbed(cachedChannel, {
                title: `[MEMBER LEFT]`,
                description: "User: " + member.user.username + " jumped out of the server.",
                color: "RED",
    ...
    client.logger.debug("[MEMBER LEFT] >> User: " + member.user.username + " jumped out of the server.")
}